...
def read_args(args):
    ...
    if args.startup_script_string:
        ...
        cpllib.simSetStringParam(const.sim_stringparam_startupscriptstring, s)

    if args.verbosity:
        s = c_char_p(args.verbosity.encode('utf-8'))
        cpllib.simSetStringParam(const.sim_stringparam_verbosity, s)

    if args.statusbar_verbosity:
        s = c_char_p(args.statusbar_verbosity.encode('utf-8'))
        cpllib.simSetStringParam(const.sim_stringparam_statusbarverbosity, s)

    if args.dlg_verbosity:
        s = c_char_p(args.dlg_verbosity.encode('utf-8'))
        cpllib.simSetStringParam(const.sim_stringparam_dlgverbosity, s)

    if args.additional_addon_script_1:
        s = c_char_p(args.additional_addon_script_1.encode('utf-8'))
        cpllib.simSetStringParam(const.sim_stringparam_additional_addonscript1, s)

    if args.additional_addon_script_2:
        s = c_char_p(args.additional_addon_script_2.encode('utf-8'))
        cpllib.simSetStringParam(const.sim_stringparam_additional_addonscript2, s)

    if args.app_arg:
        for i, arg in enumerate(args.app_arg):
            s = c_char_p(arg.encode('utf-8'))
            cpllib.simSetStringParam(const.sim_stringparam_app_arg1 + i, s)
    ...
    if args.named_param:
        for i, arg in enumerate(args.named_param):
            ...
            cpllib.simSetNamedStringParam(k, v, n)

    if args.headless or args.true_headless:
        options = const.sim_gui_headless
    else:
